setWorldLevelForGridCells(grid, worldLevel) {
    grid.forEeach(gridCol => {
        gridCol.forEeach(gridCell => {
            gridCell.worldLevel = worldLevel;
        });
    });
}

generateGrid_widthByHeightEmpty(width, height, startingTerrain = "FLOOR") {
    return Array.from({ length: width }, (_, col) =>
       Array.from({ length: height }, (_, row) => new GridCell(col, row, this, startingTerrain))
    );
}
...
generateGrid_variableCaves(smoothness = 2) {
...
generateGrid_roomsAndCorridorsRandom() {
    let grid = this.generateGrid_empty("WALL");
...
generateGrid_roomsAndCorridorsSubdivide() {
    let grid = this.generateGrid_empty("WALL");
...
export {
    ...
    generateGrid_widthByHeightEmpty,
    generateGrid_random,
    generateGrid_variable_caves,
    generateGrid_caves,
